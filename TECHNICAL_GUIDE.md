# GalaxySim — controls and technical guide
...
Choose **Land on the planet** to open the photographic sky. A small planet and
flight pad sit beside the exposure dial. Push the pad to travel, turn its ring
to rotate, and release it to stop quickly. The real sky and procedural horizon
respond together. The globe itself has no mouse controls. **WASD/arrows** move,
**Q/R** rotate and **Space** brakes while the navigator has keyboard focus.
**Enter/Escape** hides it; the location icon toggles it without resetting your
view. After six idle seconds only the globe hides and the pad dims; keyboard or pad
input brings it back. Pressing the shutter hides the whole navigator.
Quiet synthesized movement accompanies travel. A direct Metal shader renders
seeded procedural terrain, evolving clouds on a raised shell, cloud shadows,
rippled water with shallow refraction, and an atmosphere coloured by the preset.
Materials render at display resolution without CPU texture baking. These are
illustrative worlds, not physical weather or chemistry simulations.

While the planet navigator is active, press **E** for the planet workshop. Adjust cloud
motion and shape evolution, coverage, height, shadows, atmosphere, and water.
Switch planet types to compare settings across materials. Gas planets show
their own rotation, vector evolution, distortion, scale, band and wind controls;
they use animated gas bands without a separate cloud layer. **Save**
keeps global settings for future visits; **Copy JSON** shares them; **Paste JSON**
imports them. Type switching is a preview and does not change the actual planet.

Once landed, drag or scroll to look around. Choose an exposure on the existing
camera dial and press the shutter; moving during an exposure makes star trails.
The directions come from the current galaxy around your selected particle, not
an Earth star catalogue. Flying to a new location cancels an exposure; ordinary
camera movement retains its star trails. The day/night icon gently moves the
sun while keeping the galaxy, ground and camera orientation fixed. This is a
photography convenience, not a literal simulation of planetary time rotation.
Daylight selects Auto exposure (at most 0.25 seconds). Its incident-light
estimate follows the host sunlight, not where the camera points, so looking
down or away from the sun does not pump the exposure.
The back-arrow button returns to your previous selection and pause state;
Escape does the same when the navigator is closed.

[LANDING_GAME_PLAN.md](LANDING_GAME_PLAN.md) records the implementation, checks
and next steps for the landing game.
