...
orientation=bpy.data.objects.get('Cabin orientation — Y-up to Z-up')
...
for obj in scene.objects:
    ...
    if not (mesh.materials and mesh.materials[0].get('cabin_kind',0)==4):
    ...
    for tri in mesh.loop_triangles:
        ...
        color=tuple(material.get('cabin_rgb',(.1,.13,.15))) if material else (.1,.13,.15)
        kind=float(material.get('cabin_kind',0)) if material else 0
        variant=float(obj.get('screen_variant',0))
    ...
    if kind in (0,3):
        ...
        if key in cache: ao=cache[key]
            ...
            for s in samples:
                ...
                hit,normal,index,distance=bvh.ray_cast(p+n*.003,direction,.48)
                if hit is not None: occlusion+=(1-distance/.48)
...
path=ROOT+'/Sources/GalaxySim/Resources/Cabin'; os.makedirs(path,exist_ok=True)
with open(path+'/cabin.mesh','wb') as f: f.write(output)
meta={'format':'GSCABIN1','triangles':len(chunks)//3,'bytes':len(output),'objects':len(scene.objects),'ao_range':[ao_min,ao_max],'bake_seconds':time.monotonic()-started,'source':'Assets/Blender/KeplerCabin.blend','shader':'color.w contains baked local ambient visibility'}
